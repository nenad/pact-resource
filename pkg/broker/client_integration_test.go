...
func GetDetails(tb test.Broker) func(*testing.T) {
	return func(t *testing.T) {
		if err := tb.CreatePact("DetailsProvider", "DetailsConsumer", "DetailsVersion"); err != nil {
			t.Fatalf("could not create pact: %s", err)
		}

		b := broker.NewClient(tb.URL)
		got, err := b.GetDetails("DetailsProvider", "DetailsConsumer", "DetailsVersion")
		if err != nil {
		...
		want := broker.PactVersion{
			...
			ConsumerVersion: "DetailsVersion",
		}
...
func GetVersions(tb test.Broker) func(*testing.T) {
	return func(t *testing.T) {
		for i := 0; i < 10; i++ {
			if err := tb.CreatePact("VersionProvider", "VersionConsumer", fmt.Sprintf("%d", i)); err != nil {
				t.Fatalf("could not create pact: %s", err)
			}
		}

		b := broker.NewClient(tb.URL)
		got, err := b.GetVersions("VersionProvider", "VersionConsumer")
		if err != nil {
			t.Fatalf("could not get details: %s", err)
		}
